...
def seed_initial_data():
    ...
    try:
        ...
        try:
            ...
            # Concurrent worker seeded first — their data wins
            db.rollback()
            logger.info("RBAC seed skipped (another worker seeded first)")
            return
...
def _seed_rbac(db, uuid):
    """Insert permissions, roles and the bootstrap admin in one transaction."""
    from app.models import User, Role, Permission
    from app.services.auth import hash_password
